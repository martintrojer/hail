...
DROP TRIGGER provider_sync_events_account_user_insert;
DROP TRIGGER provider_sync_events_account_user_update;
DROP TRIGGER provider_accounts_refresh_token_storage_insert;
DROP TRIGGER provider_accounts_refresh_token_storage_update;
DROP TRIGGER provider_accounts_json_state_insert;
DROP TRIGGER provider_accounts_json_state_update;
...
FOR EACH ROW
WHEN NEW.sync_status NOT IN ('disabled', 'revoked', 'disconnected')
  AND (
    NEW.refresh_token_enc IS NULL
    OR length(NEW.refresh_token_enc) < 29
    OR NEW.refresh_token_ref IS NOT NULL
  )
BEGIN
...
BEFORE INSERT ON mail_accounts
FOR EACH ROW
WHEN NOT (json_valid(NEW.granted_scopes_json) AND (NEW.backfill_cursor_json IS NULL OR json_valid(NEW.backfill_cursor_json)))
BEGIN
...
FOR EACH ROW
WHEN NOT (NEW.jmap_mailbox_ids_json IS NULL OR json_valid(NEW.jmap_mailbox_ids_json))
BEGIN
  SELECT RAISE(ABORT, 'provider_message_mappings JSON state fields must be valid JSON');
END;
...
BEFORE INSERT ON provider_sync_events
FOR EACH ROW
WHEN NOT (NEW.metadata_json IS NULL OR json_valid(NEW.metadata_json))
BEGIN
  SELECT RAISE(ABORT, 'provider_sync_events JSON state fields must be valid JSON');
END;
