    ...
    T: GmailTokenSource,
{
    ...
    pub async fn get_attachment(
        ...
        message_id: &str,
        attachment_id: &str,
    ) -> Result<Vec<u8>, GmailClientError> {
        let path = format!("users/me/messages/{message_id}/attachments/{attachment_id}");
        let response: GetAttachmentResponse = self.get_json(&path, &[]).await?;
        decode_raw_rfc822(&response.data).map_err(GmailClientError::RawDecode)
    }
    ...
    async fn delete_empty(&self, path: &str) -> Result<(), GmailClientError> {
        let url = self.base_url.join(path)?;
        let mut last_retryable_error = None;
        for attempt in 1..=self.retry.attempts() {
            let token = self.token_source.bearer_token().await?;
            ...
            let error = gmail_error_from_response::<()>(response)
                .await
                .expect_err("non-success Gmail response is an error");
            if attempt < self.retry.attempts() && error.is_retryable() {
                let delay = self.retry.delay_for_retry(error.retry_after(), attempt);
                last_retryable_error = Some(error);
                if !delay.is_zero() {
                    tokio::time::sleep(delay).await;
